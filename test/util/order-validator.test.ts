...
const OUTPUT = { token: OUTPUT_TOKEN_ADDRESS, startAmount: BigNumber.from("3"), endAmount: BigNumber.from("2"), recipient: RECIPIENT };
const REACTOR = '0x1111111111111111111111111111111111111111'

function newOrder({ startTime = 5, deadline = 20, nonce = BigNumber.from(30), offerer = OFFERER, input = INPUT, output = OUTPUT, reactor = REACTOR, chainId = 1 }): DutchLimitOrder {
  ...
  describe('Testing parsed outputs', () => {
    describe('Testing endAmount', () => {
      it('Testing invalid endAmount: negative', () => {
        const order = newOrder({ output: { token: OUTPUT_TOKEN_ADDRESS, startAmount: BigNumber.from(2), endAmount: BigNumber.from(-1), recipient: RECIPIENT } });
        const validationResp = validationProvider.validate(order)
        expect(validationResp).toEqual({ errorString: 'Invalid endAmount -1', valid: false })
      })

      it('Testing invalid endAmount: too large', () => {
        const order = newOrder({ output: { token: OUTPUT_TOKEN_ADDRESS, startAmount: BigNumber.from(2), endAmount: BigNumber.from(1).shl(256), recipient: RECIPIENT } });
        const validationResp = validationProvider.validate(order)
        expect(validationResp).toEqual({ errorString: 'Invalid endAmount 115792089237316195423570985008687907853269984665640564039457584007913129639936', valid: false })
      })
    })

    describe('Testing startAmount', () => {
      it('Testing invalid startAmount: negative', () => {
        const order = newOrder({ output: { token: OUTPUT_TOKEN_ADDRESS, startAmount: BigNumber.from(-1), endAmount: BigNumber.from(2), recipient: RECIPIENT } });
        const validationResp = validationProvider.validate(order)
        expect(validationResp).toEqual({ errorString: 'Invalid startAmount -1', valid: false })
      })

      it('Testing invalid startAmount: too large', () => {
        const order = newOrder({ output: { token: OUTPUT_TOKEN_ADDRESS, startAmount: BigNumber.from(1).shl(256), endAmount: BigNumber.from(2), recipient: RECIPIENT } });
        const validationResp = validationProvider.validate(order)
        expect(validationResp).toEqual({ errorString: 'Invalid startAmount 115792089237316195423570985008687907853269984665640564039457584007913129639936', valid: false })
      })

      it('Testing startAmount >= endAmount', () => {
        const order = newOrder({ output: { token: OUTPUT_TOKEN_ADDRESS, startAmount: BigNumber.from(2), endAmount: BigNumber.from(3), recipient: RECIPIENT } });
        const validationResp = validationProvider.validate(order)
    ...
    describe('Testing recipient', () => {
      it('Testing invalid recipient.', () => {
        const order = newOrder({ output: { token: OUTPUT_TOKEN_ADDRESS, startAmount: BigNumber.from(2), endAmount: BigNumber.from(1), recipient: "0xfoo" } });
        const validationResp = validationProvider.validate(order)
    ...
    describe('Testing token', () => {
      it('Testing invalid recipient.', () => {
        const order = newOrder({ output: { token: "0xfoo", startAmount: BigNumber.from(2), endAmount: BigNumber.from(1), recipient: RECIPIENT } });
        const validationResp = validationProvider.validate(order)
