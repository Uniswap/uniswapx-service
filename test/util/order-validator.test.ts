...
describe('Testing off chain validation', () => {
  describe('Testing deadline', () => {
    ...
    it('Testing deadline longer than thirty minutes', async () => {
      const order = newOrder({ deadline: CURRENT_TIME + THIRTY_MINUTES + 1 })
      const validationResp = validationProvider.validate(order)
    ...
    it('Testing valid deadline.', async () => {
      const order = newOrder({ deadline: CURRENT_TIME + THIRTY_MINUTES })
      const validationResp = validationProvider.validate(order)
