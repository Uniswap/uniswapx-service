...
describe('FieldValidator createdAt should validator fields correctly.', () => {
...
describe('FieldValidator orderStatus should validator fields correctly.', () => {
  it('should validate field.', async () => {
  ...
  it('should invalidate field.', async () => {
    ...
    expect(validatedField.error).toBeTruthy()
    expect(validatedField.error?.details[0].message).toEqual("\"value\" must be one of [open, filled, cancelled, expired, error, unverified]")
  })
})

describe('FieldValidator orderHash should validator fields correctly.', () => {
  it('should validate field.', async () => {
  ...
  it('should invalidate field.', async () => {
    ...
    expect(validatedField.error).toBeTruthy()
    expect(validatedField.error?.details[0].message).toEqual(`\"value\" with value \"${invalidOrderHash}\" fails to match the required pattern: /^0x[0-9,a-z,A-Z]{64}$/`)
  })
})

describe('FieldValidator signature should validator fields correctly.', () => {
  it('should validate field.', async () => {
    const signature = '0x1c33da80f46194b0db3398de4243d695dfa5049c4cc341e80f5b630804a47f2f52b9d16cb65b2a2d8ed073da4b295c7cb3ccc13a49a16a07ad80b796c31b283414'
  ...
  it('should invalidate field.', async () => {
    ...
    expect(validatedField.error).toBeTruthy()
    expect(validatedField.error?.details[0].message).toEqual(`\"value\" with value \"${invalidSignature}\" fails to match the required pattern: /^0x[0-9,a-z,A-Z]{130}$/`)
  })
})

describe('FieldValidator limit should validator fields correctly.', () => {
  it('should validate field.', async () => {
...
describe('FieldValidator eth address should validator fields correctly.', () => {
  it('should validate field.', async () => {
  ...
  it('should invalidate field.', async () => {
    ...
    expect(validatedField.error).toBeTruthy()
    expect(validatedField.error?.details[0].message).toEqual(`\"value\" failed custom validation because invalid address (argument=\"address\", value=\"${invalidAddress}\", code=INVALID_ARGUMENT, version=address/5.7.0)`)
  })
})
