...
export class LambdaStack extends cdk.NestedStack {
  ...
  constructor(scope: Construct, name: string, props: LambdaStackProps) {
    ...
    this.limitOrderPostOrderLambda = new aws_lambda_nodejs.NodejsFunction(this, `LimitOrderPostOrder${lambdaName}`, {
      role: lambdaRole,
      runtime: aws_lambda.Runtime.NODEJS_18_X,
      entry: path.join(__dirname, '../../lib/handlers/index.ts'),
      handler: 'limitOrderPostOrderHandler',
      timeout: Duration.seconds(29),
      memorySize: 512,
      bundling: {
        minify: true,
    ...
    if (enableProvisionedConcurrency) {
      ...
      const limitOrderPostOrderTarget = new asg.ScalableTarget(this, `${lambdaName}-LimitOrderPostOrder-ProvConcASG`, {
        serviceNamespace: asg.ServiceNamespace.LAMBDA,
        maxCapacity: provisionedConcurrency * 2,
        minCapacity: provisionedConcurrency,
        resourceId: `function:${this.limitOrderPostOrderLambdaAlias.lambda.functionName}:${this.limitOrderPostOrderLambdaAlias.aliasName}`,
        scalableDimension: 'lambda:function:ProvisionedConcurrency',
      })

      limitOrderPostOrderTarget.node.addDependency(this.limitOrderPostOrderLambdaAlias)
      limitOrderPostOrderTarget.scaleToTrackMetric(`${lambdaName}-LimitOrderPostOrder-ProvConcTracking`, {
        targetValue: 0.8,
        predefinedMetric: asg.PredefinedMetric.LAMBDA_PROVISIONED_CONCURRENCY_UTILIZATION,
      })
